...
object ConnectorManager {
    ...
    fun tryConnectBlocks(
        connectable: DraggableBlock,
    ...
    fun DisconnectBlock(sourceBlock: DraggableBlock, viewModel: DraggableViewModel) {
        ...
        sourceBlock.connectedParent?.let {
            sourceBlock.connectedParentConnectionView?.let {
                disconnect(
                    sourceBlock.outputConnectionView!!.connector,
                    sourceBlock.connectedParentConnectionView!!.connector
                )
    ...
    fun tryConnectAndDisconnectDrag(
        ...
        ) return // Рассконекчивает, если достаточно далеко
        ...
        tryConnectBlockOnDistance(
            sourceDragBlock,
            viewModel,
            density,
            sourceDragBlock.outputConnectionView!!.connector.connectionType
        )
    }

    private fun connectBlocks(
        connectable: DraggableBlock,
        target: DraggableBlock,
        connection: ConnectionView,
        viewModel: DraggableViewModel,
        ...
        if(connectable.connectedParentConnectionView!!.connector.connectionType != ConnectorType.STRING_BOTTOM_OUTER || connectable.connectedParent!!.isInner){
            ...
            while (!stack.isEmpty()) {
                ...
                for (child in current.scope) {
                    if (child.connectedParentConnectionView!!.connector.connectionType == ConnectorType.STRING_BOTTOM_OUTER) stack.add(
                        child
                    )
                }
            }
    ...
    private fun tryConnectBlockOnDistance(
        sourceDragBlock: DraggableBlock,
        viewModel: DraggableViewModel,
        density: Density,
        ...
        val listDragBlocks =
            viewModel.blocks.value.filter { it !in sourceDragBlock.scope && (it != sourceDragBlock) }
                .toMutableList()
        ...
        when (connectionType) {
            ConnectorType.OUTPUT -> {
                ...
                nearestConnection = getNearestConnection(
                    sourceDragBlock, listDragBlocks, density,
            ...
            ConnectorType.STRING_TOP -> {
                nearestBlock = getBlockWithNearestConnection(
                    sourceDragBlock, listDragBlocks, density,
                    mutableListOf(
                        ConnectorType.STRING_BOTTOM_INNER,
                        ConnectorType.STRING_BOTTOM_OUTER
                    ), true
                )
                nearestConnection = getNearestConnection(
                    sourceDragBlock, listDragBlocks, density,
        ...
        if (CalculationsManager.getLengthFromConnections(
                sourceDragBlock,
                nearestBlock,
                sourceDragBlock.outputConnectionView!!,
                nearestConnection,
                density
            ) <= connetionLength
        ) {
            safeConnect(
                sourceDragBlock.outputConnectionView!!.connector,
                nearestConnection.connector
            )


            val sourceConX =
                with(density) { sourceDragBlock.outputConnectionView!!.positionX.toPx() }
            val sourceConY =
                with(density) { sourceDragBlock.outputConnectionView!!.positionY.toPx() }
    ...
    private fun parentDisconnectOrNot(
        ...
        sourceDragBlock.connectedParent?.let {
            if (sourceDragBlock.connectedParentConnectionView == null) {
            ...
            } else if (CalculationsManager.getLengthFromConnections(
                    sourceDragBlock,
                    sourceDragBlock.connectedParent!!,
                    sourceDragBlock.outputConnectionView!!,
                    sourceDragBlock.connectedParentConnectionView!!,
                    density
                ) > connetionLength
            ) {
                ...
                val sourceConY =
                    with(density) { sourceDragBlock.outputConnectionView!!.positionY.toPx() }
                val nearestConX =
                    with(density) { sourceDragBlock.connectedParentConnectionView!!.positionX.toPx() }
                val nearestConY =
                    with(density) { sourceDragBlock.connectedParentConnectionView!!.positionY.toPx() }
    ...
    private fun getBlockWithNearestConnection(
        ...
        if (requiresVoid) {
            for (block in listBlocks) {
                ...
                for (connection in block.inputConnectionViews) {
                    ...
                    val length = CalculationsManager.getLengthFromConnections(
                        dragBlock,
                        block,
                        dragBlock.outputConnectionView!!,
                        connection,
                        density
                    )
                    if (length < r) {
    ...
    private fun getNearestConnection(
        dragBlock: DraggableBlock,
        listBlocks: MutableList<DraggableBlock>,
        density: Density,
        connectionTypes: MutableList<ConnectorType>
    ...
    fun initConnectionsFromBlock(block: Block): MutableList<ConnectionView> {
        ...
        when (block.blockType) {
            ...
            BlockType.VARIABLE_DECLARATION -> {
                ...
                ans += mutableListOf(
                    ConnectionView(
                        castedBlock.valueInputConnector,
                        width,
                        height / 2,
                        ExtendConnectionViewType.SIDE,
                        height = defaultHeight
            ...
            BlockType.IF_BLOCK -> {
                ...
                ans += mutableListOf(
                    ...
                    ConnectionView(castedBlock.bottomConnector, cornerOffset, height),
                    ConnectionView(
                        castedBlock.innerConnector,
                        cornerOffset * 2,
                        defaultHeight,
                        ExtendConnectionViewType.INNER_BOTTOM
                    ),
                    ConnectionView(
                        castedBlock.conditionConnector,
                        width,
                        defaultHeight / 2,
                        ExtendConnectionViewType.SIDE,
                        height = defaultHeight
            ...
            BlockType.FOR_BLOCK -> {
                ...
                ans += mutableListOf(
                    ...
                    ConnectionView(
                        castedBlock.initialValueBlock,
                        width / 3 + innerPadding * 3,
                        defaultHeight / 2,
                        ExtendConnectionViewType.INNER,
                        width = defaultWidth,
                        height = defaultHeight - innerPadding * 2
            ...
            BlockType.COMPARE_NUMBERS_BLOCK -> {
                ...
                ans += mutableListOf(
                    ...
                    ConnectionView(
                        ...
                        ExtendConnectionViewType.INNER,
                        height = defaultHeight - innerPadding * 2,
                        width = defaultWidth + innerPadding * 2
                    ),
                    ConnectionView(
                        castedBlock.rightInputConnector,
                        width * 3 / 4 - defaultWidth / 2 - innerPadding * 2,
            ...
            BlockType.REPEAT_N_TIMES -> {
                ...
                ans += mutableListOf(
                    ...
                    ConnectionView(
                        castedBlock.countRepeatTimesConnector,
                        width,
                        defaultHeight / 2,
                        ExtendConnectionViewType.SIDE,
                        height = defaultHeight
            ...
            BlockType.FIXED_VALUE_AND_SIZE_LIST -> {
                ...
                ans += mutableListOf(
                    ...
                    ConnectionView(
                        castedBlock.repeatTimes,
                        width * 2 / 3 - innerPadding,
                        height / 2,
                        ExtendConnectionViewType.INNER,
                        height = defaultHeight - innerPadding * 2,
                        width = defaultWidth
            ...
            BlockType.EDIT_VALUE_BY_INDEX -> {
                ...
                ans += mutableListOf(
                    ConnectionView(
                        ...
                        width / 5,
                        height / 2,
                        extendType = ExtendConnectionViewType.INNER,
                        height = defaultHeight - innerPadding * 2,
                        width = defaultWidth
                    ),
                    ConnectionView(
                    ...
                    ConnectionView(
                        ...
                        width,
                        height / 2,
                        extendType = ExtendConnectionViewType.SIDE,
                        height = defaultHeight
                    )
                )
            }
